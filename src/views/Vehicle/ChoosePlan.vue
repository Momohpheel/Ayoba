  ...
  <div class="mt-8">
    <div class="lg:w-3/4 lg:mx-auto">
      <p class="font-semibold">Choose your preferred Insurance provider?</p>
        <div class="mt-8 box">
          <label v-for="(item, index) in underwriters" :key="index" class="container border border-gray-300 text-lg border-solid" :class="[item.id === 3 ? 'opacity-50' : '']">
            ...
            <input type="radio" v-model="underwriter" :value="item.id" :disabled="item.id === 3">
            <span class="checkmark"></span>
            <span class="tooltiptext text-sm shadow" v-if="item.id === 3">FBNGI is coming soon</span>
          </label>
        </div>
    </div>
  </div>
